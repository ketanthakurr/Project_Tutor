...
Reader_tool = TXTSearchTool(txt='D:/Gen AI Agent/recordings/combined_transcript.txt',
    config={
        "llm": {
...
# print("Similar docs")
# print(Reader_tool.run("Summarize the climate change discussion."))
...
summarise_agent = Agent(
    ...
    backstory = (
        """
        The summarise agent is adept in providing an abstractive summary of the meetings or calls given by the tool taking in count important details like facts, figures, dates etc.
  """),
    expected_output = "Summary of the meeting or call",
    # tools= [Reader_tool],
    llm = llm
)
...
question_answering_agent = Agent(
    ...
    goal = "Answer specific questions based on the content of the meetings or calls provided.",
    # allow_delegation= True,
    verbose = True,
    backstory = (
        """
...
summarise_task = Task(
    description = (
        ...
        Summarize the retrieved text, highlighting key points such as facts, figures, dates, and main discussions.
...
# Pass the input to the Crew's kickoff function
result = rag_crew.kickoff(inputs={"user_question": user_question})
print(result)
# # result = rag_crew.kickoff(inputs={"user_question": "Summarize the discussion on climate change."})
# # print(result)
